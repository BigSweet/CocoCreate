...
export default class ServerModel {
    ...
    get1SecondCategoryList(successCallback,thisObject) {
        ...
        xhr.open("POST", ServerUtils.HttpServerConstant + "c/cm/activity/getActivityCourseList?course_id=1", true);
        // 设置POST请求的请求头  
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

        xhr.send("category_id=1");//POST请求，没用参数传null
    }

    get2SecondCategoryList(successCallback,thisObject) {
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                var response = xhr.responseText;
        ...
        xhr.send("category_id=2");//POST请求，没用参数传null
    }
    get3SecondCategoryList(successCallback,thisObject) {
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                var response = xhr.responseText;
        ...
        xhr.send("category_id=3");//POST请求，没用参数传null
    }
    get4SecondCategoryList(successCallback,thisObject) {
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                var response = xhr.responseText;
        ...
        xhr.send("category_id=4");//POST请求，没用参数传null
    }
